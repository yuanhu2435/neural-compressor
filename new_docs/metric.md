...
## 1.1 Single metric
The metric supported by Neural Compressor is driven by accuracy. If users want to evaluate a specific model with a metric, users can realize it with `metric` in a yaml file. Example is provided below.

## 1.2 Multiple metrics
...
There are two usages for multi_metrics of Neural Compressor. Example is provided below.

# 2. When to use built-in metrics
If user want to get a compressed model which meet the accuracy goal, user can tune the model with neural compressor by specifying `evaluation` in a yaml file. In `evaluation` field user should specify `metric` field.

# 3. Metric support matrix
...
## Config multiple metric in a yaml file
